		...
		<div class="row ">
			<div class="col-lg-12">
				<c:forEach begin="0" end="${fn:length(listCorrectAnswer) - 1}"
					var="i">
					<h3>${listCorrectAnswer[i].num }</h3>
					<c:if test="${listCorrectAnswer[i].image != ''}">
						<img alt="" src="resource/${listCorrectAnswer[i].image }">
					</c:if>
					<br>
					<c:if test="${listCorrectAnswer[i].audioMP3 != ''}">
						<audio controls="controls">
							<source src="resource/${listCorrectAnswer[i].audioMP3 }">
						</audio>
					</c:if>
					<br>
					<p>${listCorrectAnswer[i].paragraph }</p>
					<p>${listCorrectAnswer[i].question }</p>

					<input ${listAnswer[i].answer == "A" ? 'checked' : ''}
						type="radio" name="ans[${listCorrectAnswer[i].num }]" value="A">
					<span> A. ${listCorrectAnswer[i].option1 }</span>
					<br>
					<input ${listAnswer[i].answer == "B" ? 'checked' : ''}
						type="radio" name="ans[${listCorrectAnswer[i].num }]" value="B" />
					<span> B. ${listCorrectAnswer[i].option2 }</span>
					<br>
					<input ${listAnswer[i].answer == "C" ? 'checked' : ''}
						type="radio" name="ans[${listCorrectAnswer[i].num }]" value="C" />
					<span> C. ${listCorrectAnswer[i].option3 }</span>
					<br>
					<input ${listAnswer[i].answer == "D" ? 'checked' : ''}
						type="radio" name="ans[${listCorrectAnswer[i].num }]" value="D" />
					<span> D. ${listCorrectAnswer[i].option4 }</span>
					<br>
					<c:if
