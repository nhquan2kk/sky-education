		...
		<div class="container-fluid page-body-wrapper">
			...
			<div class="main-panel">
				<div class="content-wrapper">
					<div class="row">
						<a class="my-4 text-black" href="AdminCreateVocabularyController">Create
							new vocabulary</a>
										...
										<table class="table table-bordered">
											...
											<tbody>
												<c:forEach items="${ vocabularysList}" var="item">
													<tr>
														<td>${item.vocabularyId}</td>
													<td><a
															href="DetailVocabularyController?vocabularyId=${item.vocabularyId }"
															class="">${item.name }</a></td>
														<td><img src="img/vocabulary/${item.image}" /></td>
														<td class="d-flex align-items-center"><a
															href="AdminUpdateVocabularyController?vocabularyId=${item.vocabularyId }"> <i
																class="mdi mdi-table-edit btn-icon"></i>
														</a>
															<form id="deleteForm" method="post"
																action="AdminDeleteVocabularyController">
																<input type="hidden" name="vocabularyId"
																	value="${item.vocabularyId }" />
																<button type="button" class="btn " data-toggle="modal"
																	data-target="#exampleModal-${item.vocabularyId }">
																	<i style="font-size: 30px;"
																		class="mdi mdi-delete-forever btn-icon "></i>
																</button>
																<div class="modal fade"
																	id="exampleModal-${item.vocabularyId }" tabindex="-1"
						...
						<c:if test="${numberPage == 1 }">
							<div class="btn-group" role="group" aria-label="Basic example">
								...
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminVocabularyController?pageId=1">1</a>
								</button>
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminVocabularyController?pageId=2">2</a>
								</button>
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminVocabularyController?pageId=3">3</a>
								</button>
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminVocabularyController?pageId=${numberPage +1}">Next</a>
								</button>
							</div>
						</c:if>
						<c:if test="${numberPage == maxPageId }">
							<div class="btn-group" role="group" aria-label="Basic example">
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminVocabularyController?pageId=${numberPage - 1 }">Prev</a>
								</button>
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminVocabularyController?pageId=1">1</a>
								</button>
