		...
		<div class="container-fluid page-body-wrapper">
			...
			<div class="main-panel">
				<div class="content-wrapper">
					<div class="row">
						<a class="my-4 text-black" href="AdminCreateReadingController">Create
							new reading</a>
								...
								<div class="card-body">
									<div class="table-responsive pt-3">
										<table class="table table-bordered">
											...
											<tbody>
												<c:forEach items="${ readingsList}" var="item" varStatus="index">
													<tr>
														<td>${index.count}</td>
														<td><a
															href="DetailReadingController?readingId=${item.readingId }"
															class="">${item.name }</a></td>
														<td><img src="img/reading/${item.image}" /></td>
														<td class="d-flex align-items-center"><a
															href="AdminUpdateReadingController?readingId=${item.readingId }"> <i
																class="mdi mdi-table-edit btn-icon"></i>
														</a>
															<form id="deleteForm" method="post"
																action="AdminDeleteReadingController">
																<input type="hidden" name="readingId"
																	value="${item.readingId }" />
																<button type="button" class="btn " data-toggle="modal"
																	data-target="#exampleModal-${item.readingId }">
																	<i style="font-size: 30px;"
																		class="mdi mdi-delete-forever btn-icon "></i>
																</button>
																<div class="modal fade"
																	id="exampleModal-${item.readingId }" tabindex="-1"
						...
						<c:if test="${numberPage == 1 }">
							<div class="btn-group" role="group" aria-label="Basic example">
								...
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminReadingController?pageId=1">1</a>
								</button>
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminReadingController?pageId=2">2</a>
								</button>
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminReadingController?pageId=3">3</a>
								</button>
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminReadingController?pageId=${numberPage +1}">Next</a>
								</button>
							</div>
						</c:if>
						<c:if test="${numberPage == maxPageId }">
							<div class="btn-group" role="group" aria-label="Basic example">
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminReadingController?pageId=${numberPage - 1 }">Prev</a>
								</button>
								<button type="button" class="btn btn-outline-secondary">
									<a href="AdminReadingController?pageId=1">1</a>
								</button>
